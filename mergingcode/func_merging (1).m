...
flag_merge1a = 0; % initialize
flag_merge1b = 0; % initialize
...
if moveP1.Flag_standstill==1
    ...
    if flag_merge1a == 1
        ...
        [time,jerk,tp,lambda,vp] = func_n(par(i,1),par(i,2),par(i,3),distance);
        moveP2.jerk = jerk;
        moveP2.vp = vp;
        moveP2.tp = tp;
    ...
    % passes the first intermediary point.
        ...
        time12i = jerkdistinv(jerk, tp, lambda, abs(p2(i)-p1(i)));
        moveP2.i = i;
            moveP2.Flag_standstill = 0;
            moveP2.jerk = jerk;
            moveP2.vp = vp;
    ...
    %% If two consecutive moves are diagonal, we may still be able to 
    %  make an opportunistic merge.
    else %%%%% THIS HAS BEEN ADDED!!!
        distance = abs(p2(1)-p1(1));
        [time12_1,~,tp1,~,~] = func_n(par(1,1),par(1,2),par(1,3),distance);
        distance = abs(p2(2)-p1(2));
        [time12_2,~,tp2,~,~] = func_n(par(2,1),par(2,2),par(2,3),distance);
        if time12_1 - time12_2 >= 2*tp1 && sign(p3(1)-p2(1)) == sign(p2(1)-p1(1))
            moveP2.i = 1;
            moveP2.Flag_standstill = 0;
            distance = abs(p3(1)-p1(1));
            [time13_1,jerk1,tp1,lambda1,vp1] = func_n(par(1,1),par(1,2),par(1,3),distance);
            moveP2.jerk = jerk1;
            moveP2.vp = vp1;
            moveP2.tp = tp1;
            moveP2.lambda = lambda1; 
            moveP2.xi = abs(p2(1)-p1(1));
            moveP2.ti = jerkdistinv(moveP2.jerk,moveP2.tp,moveP2.lambda,moveP2.xi);
            moveP2.time_remi = time13_1 - moveP2.ti;
            if moveP2.ti > (moveP2.lambda+2*moveP2.tp)
                moveP2.Flag_dec = 1;
            else
                moveP2.Flag_dec=0;
            ...
            time_P12 = moveP2.ti;
        elseif time12_2 - time12_1 >= 2*tp2 && sign(p3(2)-p2(2)) == sign(p2(2)-p1(2))
            moveP2.i = 2;
            moveP2.Flag_standstill = 0;
            distance = abs(p3(2)-p1(2));
            [time13_2,jerk2,tp2,lambda2,vp2] = func_n(par(2,1),par(2,2),par(2,3),distance);
            moveP2.jerk = jerk2;
            moveP2.vp = vp2;
            moveP2.tp = tp2;
            moveP2.lambda = lambda2; 
            moveP2.xi = abs(p2(2)-p1(2));
            moveP2.ti = jerkdistinv(moveP2.jerk,moveP2.tp,moveP2.lambda,moveP2.xi);
            moveP2.time_remi = time13_2 - moveP2.ti;
            if moveP2.ti > (moveP2.lambda+2*moveP2.tp)
                moveP2.Flag_dec = 1;
            else
                moveP2.Flag_dec=0;
